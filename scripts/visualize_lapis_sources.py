...
"""
SOURCE_REPORT_CSS = REPORT_BASE_CSS + """
    .bars {
      display: grid;
      gap: 8px;
    }
    .bar-row {
      display: grid;
      grid-template-columns: minmax(150px, 1fr) minmax(120px, 2fr) 20ch;
      gap: 10px;
      align-items: center;
      min-height: 30px;
      font-size: 0.95rem;
...
"""
TIMELINE_REPORT_CSS = REPORT_BASE_CSS + """
    .chart-grid {
      display: grid;
      grid-template-columns: 1fr;
      gap: 16px;
...
<head>
  ...
  <title>{title}</title>
  <style>{DASHBOARD_INDEX_CSS}</style>
</head>
<body>
  <main>
    <h1>{title}</h1>
...
def render_timeline_html(
    ...
    const colors = [
      '#176b62', '#3867a8', '#b45d36', '#247a4d', '#7b5fb0', '#2f7f91',
      '#8a6f2d', '#a33f3f', '#4f7d52', '#597e9f', '#8b5e76', '#5f756c',
      '#6f7f37', '#8a6a40', '#3d748a',
    ];
    const state = {
      lang: 'en',
      period: 'day',
    ...
    function continuousPeriodLabels(startLabel, endLabel) {
      const labels = [];
      if (state.period === 'month') {
        const cursor = parseMonthLabel(startLabel);
      ...
      if (state.period === 'week') {
        const cursor = parseWeekLabel(startLabel);
        const end = parseWeekLabel(endLabel);
        while (cursor <= end) {
          labels.push(formatWeekLabel(cursor));
          cursor.setUTCDate(cursor.getUTCDate() + 7);
        }
        return labels;
      }
      const cursor = parseDayLabel(startLabel);
      const end = parseDayLabel(endLabel);
      while (cursor <= end) {
    ...
    function fillMissingPeriods(periods) {
      if (!periods.length) return periods;
      const byPeriod = new Map(periods.map((period) => [period.period, period]));
      return continuousPeriodLabels(periods[0].period, periods[periods.length - 1].period)
      ...
      const chartHtml = `
        <div class="column-chart" style="--bar-count:${periods.length}">
          ${periods.map((period, index) => {
            const height = period.total > 0 ? Math.max((period.total / max) * 100, 2) : 0;
            const barStyle = period.total > 0
            ...
            return `
              <div class="time-column">
                <div class="time-bar-slot">
                  <div class="stack-bar" style="${barStyle}">
                    ${pieces.map((piece) => {
    ...
    function render() {
      ...
      renderStackChart(fillMissingPeriods(periods), sourceLabels(sources));
      renderTable(periods);
